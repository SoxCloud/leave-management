...
import { Learner, LeaveRequest, LeaveStatus, LeaveType } from '../../types';
import {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Days Accrued', value: accrued, icon: Calendar, color: 'text-blue-400' },
          { label: 'Annual Taken', value: annualTaken, icon: Clock, color: 'text-amber-400' },
          { label: 'Current Balance', value: balance, icon: Award, color: 'text-emerald-400' },
          { label: 'Attendance', value: `${attendancePct}%`, icon: Activity, color: attendancePct >= 90 ? 'text-emerald-400' : 'text-red-400' },
        ].map(stat => (
          <div key={stat.label} className="rounded-xl border border-slate-800/60 bg-slate-900/50 p-4">
          ...
          <div className="space-y-2 max-h-[200px] overflow-y-auto">
            {learnerLeaves.length === 0 ? (
              ...
              learnerLeaves.slice(0, 10).map(lr => (
                <div key={lr.id} className="flex items-center justify-between px-3 py-2 bg-slate-800/30 rounded-lg">
